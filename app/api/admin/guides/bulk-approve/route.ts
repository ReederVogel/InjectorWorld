...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await approveContentUpload(payload, 'guides', {
      batch,
    ...
    payload.logger.error(`[guides bulk approve] ${err?.message ?? err}`)
    return serverError('admin/guides/bulk-approve', err, 'Approve failed.')
  }
}
