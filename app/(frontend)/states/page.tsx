...
type StateRow = { name: string; slug: string; isLive: boolean; providerCount: number }
...
export default async function StatesIndexPage() {
  ...
  const soon = states.filter((s) => !s.isLive)
                ...
                {live.map((s) => (
                  ...
                  >
                    <div>
                      ...
                      {s.providerCount > 0 && (
                        <div className="text-caption text-ink-tertiary mt-0.5">{s.providerCount.toLocaleString()}+ providers</div>
                      )}
                    </div>
