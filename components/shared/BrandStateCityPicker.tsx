...
import type { BrandStateEntry, BrandCityEntry } from '@/lib/brand-queries'
...
export function BrandStateCityPicker({ brandSlug, brandName, states, allCities }: Props) {
  const router = useRouter()
  const [selectedState, setSelectedState] = useState<BrandStateEntry | null>(null)
  ...
  function handleStateSelect(state: BrandStateEntry) {
  ...
  function handleCitySelect(city: BrandCityEntry) {
    setIsOpen(false)
    router.push(`/brands/${brandSlug}/${city.stateSlug}/${city.slug}`)
  }
    ...
    <div className="rounded-2xl border border-border bg-surface-warm p-6 md:p-8">
      <div className="mb-6">
        <h2 className="font-serif text-h2 text-ink-primary leading-tight">
          Find {brandName} clinics near you
        </h2>
