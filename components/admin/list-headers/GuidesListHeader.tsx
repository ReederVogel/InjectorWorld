...
type GuideUploadItem = { id: number; slug: string; title: string; status: 'created' | 'updated' }
type GuideUploadReport = {
  batch: string
  total: number
  created: number
  updated: number
...
export function GuidesListHeader() {
  ...
  return (
    <ListHeader
      ...
      extra={<GuidesBulkUpload onAfterChange={refresh} />}
    />
  )
}

function GuidesBulkUpload({ onAfterChange }: { onAfterChange: () => void }) {
  const [open, setOpen] = useState(false)
  const [file, setFile] = useState<File | null>(null)
  const [busy, setBusy] = useState('')
  const [msg, setMsg] = useState('')
  const [report, setReport] = useState<GuideUploadReport | null>(null)
  ...
  async function upload() {
    ...
    try {
      ...
      const res = await fetch('/api/admin/guides/bulk-upload', {
        method: 'POST',
        credentials: 'include',
      ...
      setReport(json.report as GuideUploadReport)
  ...
  async function approve(opts: { batch?: string; id?: number }) {
    ...
    setMsg('')
    try {
      const res = await fetch('/api/admin/guides/bulk-approve', {
        method: 'POST',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
      ...
      setMsg(`Approved ${json.approved} guide${json.approved === 1 ? '' : 's'}.`)
      onAfterChange()
      setReport((prev) => {
        if (!prev) return prev
  ...
  return (
    <div style={{ border: '1px solid #3FA68A40', borderRadius: 8, padding: 14, background: '#3FA68A0d' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
        <strong style={{ fontSize: 14, color: '#0B1B34' }}>Bulk upload guides (JSON)</strong>
        <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
          <a
            href="/templates/guides-template.json"
            download
      ...
      {open && (
        <ol style={{ fontSize: 12.5, opacity: 0.85, margin: '10px 0 0', paddingLeft: 18, lineHeight: 1.6 }}>
          ...
          <li>Each guide is staged as a draft, not live yet.</li>
