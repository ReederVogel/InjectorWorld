    ...
    <section className="bg-surface section-pad">
      <div className="max-canvas">
        <div className="flex items-end justify-between gap-6 mb-10 md:mb-14 flex-wrap">
          ...
          <Link href="/clinics" className="group inline-flex items-center gap-2 text-body-lg font-semibold text-brand-accent hover:underline">
            See all featured
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="group-hover:translate-x-0.5 transition"><polyline points="9 18 15 12 9 6" /></svg>
          </Link>
        </div>
  ...
  return (
    <article className="group card-premium bg-surface-canvas border border-border rounded-2xl overflow-hidden flex-shrink-0 w-[80vw] max-w-[340px] md:w-auto snap-start">
      ...
      <div className="relative h-[220px] md:h-[240px] bg-surface overflow-hidden">
        ...
        <span className="absolute top-4 left-4 inline-flex items-center gap-1.5 bg-brand-accent text-surface-canvas text-[10px] font-bold tracking-wider px-2.5 py-1 rounded-control uppercase shadow-md">
      ...
      <div className="p-6 md:p-7">
        ...
        <div className="flex items-start gap-3 mb-4">
          <div className="relative w-16 h-16 rounded-full overflow-hidden bg-surface flex-shrink-0 -mt-14 border-4 border-surface-canvas shadow-md transition-all duration-200 group-hover:ring-2 group-hover:ring-brand-accent group-hover:ring-offset-2">
        ...
        <div className="flex flex-wrap gap-1.5 mb-5">
          ...
          {extraCount > 0 && (
            <span className="text-[11px] px-2.5 py-1 rounded-control bg-surface text-ink-tertiary font-medium border border-border">+{extraCount} more</span>
          )}
        </div>
        ...
        {p.loyaltyPrograms && p.loyaltyPrograms.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-3">
            {(p.loyaltyPrograms as string[]).map((prog) => {
              ...
              return (
                <span key={prog} className="text-[9px] px-2 py-0.5 rounded-control bg-surface border border-border text-ink-tertiary font-semibold uppercase tracking-wide">
                  {labels[prog] ?? prog}
                </span>
              )
            })}
        ...
        <div className="flex gap-2">
          <Link href={`/injectors/${p.clinic.stateSlug}/${p.clinic.citySlug}/${p.slug}#book`} className="flex-1 bg-brand-primary text-surface-canvas rounded-control py-2.5 text-body-sm font-medium text-center hover:opacity-90 transition">Book consult</Link>
          <Link href={`/injectors/${p.clinic.stateSlug}/${p.clinic.citySlug}/${p.slug}`} className="flex-1 border border-border rounded-control py-2.5 text-body-sm font-medium text-center text-ink-primary hover:bg-surface hover:border-brand-accent transition">View profile</Link>
        </div>
      </div>
    </article>
  )
