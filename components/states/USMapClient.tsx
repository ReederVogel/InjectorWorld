...
// Real geographic SVG paths from WebsiteBeaver interactive US map
...
// Approximate label positions (cx, cy) for each state abbreviation
const STATE_LABELS: Record<string, [number, number]> = {
  AL: [831, 520], AK: [290, 620], AZ: [350, 470], AR: [715, 468],
  CA: [235, 420], CO: [490, 370], CT: [1030, 285], DE: [1003, 340],
  FL: [910, 610], GA: [872, 507], HI: [490, 650], ID: [375, 232],
  IL: [760, 375], IN: [815, 370], IA: [670, 310], KS: [605, 385],
  KY: [820, 415], LA: [710, 560], ME: [1070, 195], MD: [975, 355],
  MA: [1055, 267], MI: [820, 268], MN: [680, 215], MS: [760, 505],
  MO: [700, 400], MT: [450, 195], NC: [940, 435], ND: [570, 195],
  NE: [590, 310], NV: [290, 340], NH: [1044, 230], NJ: [1008, 310],
  NM: [430, 490], NY: [970, 258], OH: [862, 310], OK: [630, 450],
  OR: [265, 250], PA: [930, 300], RI: [1052, 279], SC: [948, 490],
  SD: [565, 265], TN: [820, 452], TX: [618, 545], UT: [385, 355],
  VT: [1020, 242], VA: [960, 390], WA: [280, 160], WV: [914, 370],
  WI: [760, 265], WY: [475, 295],
}

type Tooltip = { x: number; y: number; state: StateData } | null
...
export function USMapClient({ states }: { states: StateData[] }) {
  ...
  const stateMap = useMemo(() => {
    ...
    for (const s of states) m.set(s.abbr.toUpperCase(), s)
    return m
  }, [states])

  const matchedAbbrs = useMemo(() => {
  ...
  const handleMouseEnter = (abbr: string, e: React.MouseEvent<SVGPathElement>) => {
    ...
    setTooltip({ x: svgPt.x, y: svgPt.y, state: s })
  }
  ...
  const getColor = (abbr: string): string => {
    const s = stateMap.get(abbr)
    if (!s) return '#CBD5E1'
    if (matchedAbbrs && !matchedAbbrs.has(abbr)) return '#EEF1F5'
    if (abbr === hovered) return s.isLive ? '#2d9175' : '#94A3B8'
    return s.isLive ? '#3FA68A' : '#CBD5E1'
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-4 text-body-sm">
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-3 w-3 rounded-full" style={{ background: '#3FA68A' }} />
          <span className="text-ink-secondary">Live now</span>
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-3 w-3 rounded-full" style={{ background: '#CBD5E1' }} />
          <span className="text-ink-secondary">Coming soon</span>
        </span>
      </div>

      {/* Map */}
      <div className="relative w-full overflow-hidden rounded-xl border border-border bg-[#FAF7F2]">
        <svg
        ...
        >
          ...
          {Object.entries(STATE_LABELS).map(([abbr, [cx, cy]]) => {
            const dimmed = matchedAbbrs && !matchedAbbrs.has(abbr)
            return (
              <text
                key={abbr}
                x={cx}
          ...
          {/* Tooltip */}
          {tooltip && (
            <g transform={`translate(${tooltip.x},${tooltip.y - 14})`} style={{ pointerEvents: 'none' }}>
              <rect x="-60" y="-28" width="120" height="32" rx="6" fill="#0B1B34" opacity="0.92" />
              <text x="0" y="-14" textAnchor="middle" dominantBaseline="middle" fontSize="11" fontWeight="700" fontFamily="Inter, sans-serif" fill="#ffffff">
                {tooltip.state.name}
              </text>
              <text x="0" y="0" textAnchor="middle" dominantBaseline="middle" fontSize="10" fontFamily="Inter, sans-serif" fill="#94A3B8">
