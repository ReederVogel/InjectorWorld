  ...
  return (
    <>
      <div ref={scrollRef} className="flex md:grid md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6 overflow-x-auto snap-x snap-mandatory hide-scrollbar -mx-5 md:mx-0 px-5 md:px-0 pb-2">
        {cases.map((c) => (
          <article key={c.id} className="card-premium flex-shrink-0 w-[300px] md:w-auto snap-start bg-surface-canvas border border-border rounded-2xl overflow-hidden">
            <BeforeAfterSlider beforeUrl={c.beforePhotoUrl} afterUrl={c.afterPhotoUrl} alt={c.caseTitle} />
            <div className="p-5">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-[10px] px-2.5 py-1 rounded-pill bg-brand-accent-soft text-brand-primary font-semibold uppercase tracking-wider">{c.treatmentTag}</span>
              ...
              <div className="text-body-sm text-ink-primary mb-1">
                Treated by{' '}
                {c.provider?.slug ? (
                  <Link href={`/injectors/${c.provider.slug}`} className="font-medium text-brand-accent hover:underline">{c.provider.fullName}</Link>
                ) : (
