 ...
 * directory has same-name clinics at the same zip). The bulk importer sets
 * the slug itself (lib/import/import-data.ts); this covers admin/dashboard
 * creates and any update that clears the slug.
 *
 * LOCKED 2026-08-04 (founder-mandated): name-zip only. City and state are
 * already path segments (`/clinics/[state]/[city]/[slug]`), so they must
 * NOT be folded into the slug itself — a same-day migration to that shape
 * was reverted after founder pushback. Keep this in step with
 * lib/import/import-data.ts and any one-off batch import script.
 */
